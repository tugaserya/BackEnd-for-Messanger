    ...
    try{
        ...
        if (user_id.rows[0].id === message.rows[0].sender_id) {
            ...
            const message = {
                ...
                file_name: updated_message.rows[0].originalfile != "" ? updated_message.rows[0].originalfile : "",
                fyleType: updated_message.rows[0].file_type != "" ? updated_message.rows[0].file_type : "",
...
async ArchiveMessage(message_data, login){
    ...
    if (user_id.rows[0].id === message.rows[0].sender_id || user_id.rows[0].id === message.rows[0].recipient_id) {
        ...
        if (message.rows.length > 0) {
            if(message.rows[0].file != "") {
                await fs.unlink(path.join(__dirname, '../../uploads/' + message.rows[0].file_type + '/' + message.rows[0].file), (err => {
